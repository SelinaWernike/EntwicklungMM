...
public class PrefabRan : MonoBehaviour
{
    ...
    void Start()
    {
       ...
       GameObject go = Instantiate(myPrefab,new Vector3(Random.Range(-3.0f,3.0f),0.1f,Random.Range(-2.0f,2.0f)), Quaternion.Euler(0,Random.Range(0,360),0)) as GameObject;
       if(Random.Range(0,4) <= 2.0f) {
        go.transform.parent = parentTarget.transform;
